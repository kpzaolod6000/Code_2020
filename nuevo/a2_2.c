...
int main()	
{
		...
		for (j=0;j<I[i].n;j++){
			...
			if (I[i].rhs[j][temp_ind] == '.'){
				...
				if (I[i].rhs[j][temp_ind-1] == 'S' && i==1)
					sprintf(slr[i][strchr(terminals,'$')-terminals],"aceptar");
				else{
					//printf("--->%d %d %c<---\n",i,j,I[i].lhs[j]);
					//printf("Here..j..%c\n",I[i].lhs[j]);
					...
					for (ha=0; ha<m2 ; ha++){
						...
						if (strlen(slr[i][tempo_store]) > 0){
							if (slr[i][tempo_store][0] == 's')
								printf("\nSR-Se produjo un conflicto....\n");
							else
								printf("\nRR-Se produjo un conflicto....\n");
						}
						else
	...
	while(i<input_len){
		...
		if (strchr(terminals,input[i])){
			...
			printf("Símbolo desconocido encontrado...");
			break;
		}

		if (stack[top]=='1' && input[i]=='$' && strcmp(slr[state][temp32],"aceptar") == 0 ) {
